...
var i=1===parseInt(t.decisionTree.predict(a[r]))?"wellPerformed":"notWellPerformed",n="users="+a[r].users+"&sessionId="+window.sessionStorage.getItem("sessionId")+"&classification="+i
...
return i}(s,"users")
for(var h=-1/0,r=0;r<s.length;r++)s[r].status>h&&(h=s[r].status)
0==h&&(h=-1)
for(var r=0;r<s.length;r++){var p=s[r],n="users="+p.users+"&sessionId="+p.sessionId+"&status="+Math.floor(p.status/h*100)
...
for(var h=u[c],p="",m=0,v=u[c].length;m<v;m++)p+=m<v-1?u[c][m].toFixed(4)+", ":u[c][m].toFixed(4)
var a=t.nodes.get(d).size,g=t.nodes.get(d).info,b=t.nodes.get(d).featureCount,f=t.nodes.get(d).label+""
if(b++,"INPUT"===$(g).prop("tagName")){g=$(g).val(),g="["+g+"]",g=JSON.parse(g)
for(var y="<table class='table' style='width: 100%;'><tr>",O=getHeaders(u[c]),m=0,v=g.length;m<v;m++)y+="<th>"+O[m]+"</th>"
y+="<th>Notes</th>",y+="</tr><tr>",h=getInputVector(h)
for(var m=0;m<h.length;m++)y+="<td>"+h[m]+"</td>"
analysisObjects.miners.neuralNTreeNotes&&analysisObjects.miners.neuralNTreeNotes[c]?y+="<td>"+analysisObjects.miners.neuralNTreeNotes[c]+"</td>":u[c].users?y+="<td>"+u[c].users+"</td>":u[c].id?y+="<td>"+u[c].id+"</td>":y+="<td>No notes</td>",y+="</tr>",y+="</table>"}else{var j=g.substring(0,g.length-8),N="<tr>"
h=getInputVector(h)
for(var m=0,v=h.length;m<v;m++)N+="<td>"+h[m]+"</td>"
...
var y=j+N}g+="<br>"+JSON.stringify(h),f=f.split("\n")[0]+"\nn = "+b,a+=.3,t.nodes.update({id:d,size:a,points:t.nodes.get(d).currentPoints+"<hr>"+y,info:y,featureCount:b,label:f})}for(var T=[],c=0;c<l.length;c++)for(var m=0;m<l.length;m++)if(c!==m&&l[c]&&l[m]&&l[c].length>1&&l[m].length>1){var o=t.silhouette(l[c],l[m])
...
var v=NNTFunctions.util.similarity(m,l)
null===d&&(d=v),null===h&&(h=v),d<v&&(d=v),h>v&&(h=v)}this.levelCount=t.length
for(var g=getHeaders(a[0]),s=0,n=t.length;s<n;s++)for(var i=0,b=t[s].length;i<b;i++)if(null!==t[s][i]){for(var f="<input type='text' style='width: 290px;' value='",y=NNTFunctions.util.convertRange(t[s][i].point,[-2,2],[this.neuralNTree.neuralNTree.min,this.neuralNTree.neuralNTree.max]),O="<div class='nnt-point-container'>",u=0,c=y.length;u<c;u++)f+=u!==c-1?y[u].toFixed(4)+", ":y[u].toFixed(4),O+="<div class='input-group'><span class='input-group-addon'>"+g[u]+"</span><input type='text' id='node-index_"+u+"_"+t[s][i].index+"' class='node-index' onchange='pointChange(this)'></div>"
O+="</div>",f+="' placeholder='point' id='node-index_"+t[s][i].index+"' class='node-index' onchange='pointChange(this)'>"
var v=NNTFunctions.util.similarity(y,l)
v=parseInt(NNTFunctions.util.convertRange([v],[d,h],[0,255]))
var j=t[s][i].isTerminal?"Terminal "+t[s][i].index:"Node "+t[s][i].index
v<0?v=0:v>255&&(v=255)
var N=255-v,T=v,S=255-v,I="rgba("+N+","+T+","+S+",1)"
t[s][i].mergeIndex&&(this.mergeIndices[t[s][i].mergeIndex]=!0),this.nodes.add({id:t[s][i].index,label:t[s][i].index,level:s,info:f,points:O,size:25,shape:"square",featureCount:0,color:I,nodeHeader:j,cid:t[s][i].mergeIndex,currentPoints:O}),this.lastNodes.push({id:t[s][i].index,label:t[s][i].index,level:s,info:f,points:O,size:25,shape:"square",featureCount:0,color:I,nodeHeader:j,cid:t[s][i].mergeIndex,currentPoints:O}),null!==t[s][i].parent&&this.edges.add({from:t[s][i].index,to:t[s][i].parent.index})}var D=document.getElementById("neural-n-tree-visual"),w={nodes:this.nodes,edges:this.edges},x={edges:{smooth:{type:"cubicBezier",forceDirection:"vertical",roundness:.4}},layout:{hierarchical:{direction:"UD"}},physics:!1}
this.visual=new vis.Network(D,w,x),this.visual.on("click",function(t){var a=t.nodes,s=e.nodes.get(a)
...
var h=!1
d!=$("#anomaly-threshold").val()&&(h=!0),isNaN(d)||$("#anomaly-threshold").val(d),r="<table id='a-table' class='table'><thead><tr>"
...
r+="</tr>"}r+="</tbody></table>",$("#anomaly-table").html(r),$("#a-table").tablesorter(),hideLoader(),h&&$("#apply-anomaly").click()}),hideLoader(),analysisObjects.miners.anomaly){t.anomaly=analysisObjects.miners.anomaly
...
for(var d in e)for(var p=e[d],m=0;m<p.length;m++){var v=p[m],g=v.itemSet.join(),b=v.support
h+="<tr><td>"+g+"</td><td>"+b+"</td></tr>"}h+="</tbody></table>"
var f={},y={nodes:s,edges:r},O=document.getElementById("apriori-table")
O.style.width="900px",O.style.height="900px",new vis.Network(O,y,f),$("#apriori-support-div").html(h),$("#freq-table").tablesorter(),$("#asso-table").tablesorter()}
