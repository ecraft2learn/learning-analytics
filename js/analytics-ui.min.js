...
for(var a=0,i=t.length;a<i;a++)for(var r=0,b=t[a].length;r<b;r++)if(null!==t[a][r]){for(var v="<input type='text' style='width: 290px;' value='",g=NNTFunctions.util.convertRange(t[a][r].point,[-2,2],[this.neuralNTree.neuralNTree.min,this.neuralNTree.neuralNTree.max]),o=0,u=g.length;o<u;o++)v+=o!==u-1?g[o].toFixed(4)+", ":g[o].toFixed(4)
v+="' placeholder='point' id='node-index_"+t[a][r].index+"' class='node-index' onchange='pointChange(this)'>"
...
t[a][r].mergeIndex&&(this.mergeIndices[t[a][r].mergeIndex]=!0),this.nodes.add({id:t[a][r].index,label:t[a][r].index,level:a,info:v,size:25,shape:"square",featureCount:0,color:N,nodeHeader:y,cid:t[a][r].mergeIndex}),null!==t[a][r].parent&&this.edges.add({from:t[a][r].index,to:t[a][r].parent.index})}var T=document.getElementById("neural-n-tree-visual"),S={nodes:this.nodes,edges:this.edges},D={edges:{smooth:{type:"cubicBezier",forceDirection:"vertical",roundness:.4}},layout:{hierarchical:{direction:"UD"}},physics:!1}
