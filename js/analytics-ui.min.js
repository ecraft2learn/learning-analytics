...
c=u[o].length>0?c/u[o].length:0,s[i][o]="id"===o||o in a?u[o][0]:c}}return s}function groupBySum(e,t){for(var a=analysisObjects.miners.ignoredAttributes,s=[],r={},i=0;i<e.length;i++){var n=e[i],l=n[t]
if(r[l])for(var o in n)o!==t&&r[l][o].push(n[o])
else{r[l]={}
for(var o in n)o!==t&&(r[l][o]=[],r[l][o].push(n[o]))}}for(var o in r)s.push(r[o])
for(var i=0;i<s.length;i++){var u=s[i]
for(var o in u){for(var c=0,d=0;d<u[o].length;d++)isNaN(u[o][d])||(c+=parseFloat(u[o][d]))
s[i][o]="id"===o||o in a?u[o][0]:c}}return s}function normalize(e,t){for(var a=analysisObjects.miners.ignoredAttributes,s=[],r=0;r<e.length;r++){var i={},n=(e[r][t],e[r][t])
...
c.setHours(p),c.setMinutes(h),c=a(c),r&&i&&c>=r&&c<=i?n.push(u):r&&!i&&c>=r?n.push(u):!r&&i&&c<=i&&n.push(u)}return n}return null}function pointChange(e){var t=e.id,a=$("#"+t).val(),s=t.split("_")[1],r="["+a+"]",i=JSON.parse(r)
...
0==i&&(i=1e-7),0==n&&(n=1e-7)}catch(e){return void alert("Not a valid Neural N-Tree parameter")}t.neuralNTree=new NeuralNTree(a,i,n,s,r),t.settings={},t.settings.size=a,t.settings.alpha=s,t.settings.dimensions=r,t.settings.min=i,t.settings.max=n,analysisObjects.miners.neuralNTreeSettings=t.settings}if(analysisObjects.miners.curDataSet||analysisObjects.miners.groupDataSet||analysisObjects.miners.reducedTimeDataSet||analysisObjects.miners.dataSet){for(var l=[],o=analysisObjects.miners.curDataSet||analysisObjects.miners.groupDataSet||analysisObjects.miners.reducedTimeDataSet||analysisObjects.miners.dataSet,u=0;u<o.length;u++){var c=t.neuralNTree.neuralNTree.cluster(getInputVector(o[u])).index
l[c]||(l[c]=[]),l[c].push(getInputVector(o[u]))
for(var d=o[u],m="",h=0,p=o[u].length;h<p;h++)m+=h<p-1?o[u][h].toFixed(4)+", ":o[u][h].toFixed(4)
var a=t.nodes.get(c).size,b=t.nodes.get(c).info,g=t.nodes.get(c).featureCount,v=t.nodes.get(c).label+""
...
var y=O+j}b+="<br>"+JSON.stringify(d),v=v.split("\n")[0]+"\nn = "+g,a+=.3,t.nodes.update({id:c,size:a,info:y,featureCount:g,label:v})}for(var N=[],u=0;u<l.length;u++)for(var h=0;h<l.length;h++)if(u!==h&&l[u]&&l[h]&&l[u].length>1&&l[h].length>1){var T=t.silhouette(l[u],l[h])
...
for(var a=0,i=t.length;a<i;a++)for(var r=0,l=t[a].length;r<l;r++)if(null!==t[a][r])for(var o=0,u=t[a][r].point.length;o<u;o++)n[o]>NNTFunctions.util.convertRange([t[a][r].point[o]],[-2,2],[this.neuralNTree.neuralNTree.min,this.neuralNTree.neuralNTree.max])[0]&&(n[o]=NNTFunctions.util.convertRange([t[a][r].point[o]],[-2,2],[this.neuralNTree.neuralNTree.min,this.neuralNTree.neuralNTree.max])[0]),0==n[o]&&(n[o]=1e-8)
for(var c=null,d=null,a=0,m=t.length;a<m;a++)for(var r=0,l=t[a].length;r<l;r++)if(null!==t[a][r]&&t[a][r]!==this.neuralNTree.neuralNTree.rootNode&&t[a][r]!==this.neuralNTree.neuralNTree.rootNode.left&&t[a][r]!==this.neuralNTree.neuralNTree.rootNode.right){var h=t[a][r].point
...
for(var a=0,i=t.length;a<i;a++)for(var r=0,b=t[a].length;r<b;r++)if(null!==t[a][r]){for(var g="<input type='text' style='width: 290px;' value='",v=NNTFunctions.util.convertRange(t[a][r].point,[-2,2],[this.neuralNTree.neuralNTree.min,this.neuralNTree.neuralNTree.max]),o=0,u=v.length;o<u;o++)g+=o!==u-1?v[o].toFixed(4)+", ":v[o].toFixed(4)
...
s.push({input:getInputVectorTextAllAttributes(i[n]),status:o})}for(var u=t.anomaly.outlier.probs,c=0,n=0;n<u.length;n++)c+=u[n]
u.length>0&&(c/=u.length),isNaN(c)||$("#anomaly-threshold").val(c),r="<table id='a-table' class='table'><thead><tr>"
...
t.chartOptions=n
var o=new Chart(e,n),u=new Chart(a,l)
t.radarChart=o,t.lineChart=u,hideLoader()})},initApriori.prototype.createUI=function(){var e=$(analysisObjects.uiConstants.wrapper),t=this,a=isChrome()?analysisObjects.urls.templateUrl+"apriori.html":"templates/apriori.html"
...
i+="<tr><td>"+u+"</td><td>"+c+"</td></tr>"}i+="</tbody></table>",$("#apriori-table").html(a),$("#apriori-support-div").html(i),$("#freq-table").tablesorter(),$("#asso-table").tablesorter()}
