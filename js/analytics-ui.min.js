...
a+="<td><select id='option-"+e[t].hiddenId+"' onchange='selectPerformance(this);'><option value='-1'>Not performing well</option><option selected='selected' value='0'>Medium permormance</option><option value='1'>Performing well</option></select></td>",a+="</tr>"}a+="</tbody></table>",$("#data-set-table").html(a),$("#attribute-table").tablesorter(),new initCharts}function selectPerformance(e){for(var t=e.id.split("-")[1],a=e.value,s=analysisObjects.miners.curDataSet||analysisObjects.miners.groupDataSet||analysisObjects.miners.reducedTimeDataSet||analysisObjects.miners.dataSet,r=0;r<s.length;r++)if(s[r].hiddenId==t){s[r].wellPerformed=a
...
for(var e=[],t=0;t<this.data.length;t++)this.data[t].wellPerformed?this.data[t].wellPerformed=parseInt(this.data[t].wellPerformed):this.data[t].wellPerformed=0
...
t.visual=new vis.Network(u,c,d),t.visual.on("click",function(e){var a=e.nodes,s=t.nodes.get(a)
