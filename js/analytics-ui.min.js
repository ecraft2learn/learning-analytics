...
var y=O+j}b+="<br>"+JSON.stringify(d),g=g.split("\n")[0]+"\nn = "+v,a+=.3,t.nodes.update({id:u,size:a,info:y,featureCount:v,label:g})}for(var N=[],c=0;c<l.length;c++)for(var h=0;h<l.length;h++)if(c!==h&&l[c]&&l[h]&&l[c].length>1&&l[h].length>1){var T=t.silhouette(l[c],l[h])
...
hideLoader()}),$("#backward").click(),$("#forward").click(),$("#cluster").click(),$("#reset-cluster").on("click",function(e){t.neuralNTree=null,analysisObjects.miners.neuralNTree&&analysisObjects.miners.neuralNTree.reset()}),hideLoader()})},initCluster.prototype.createSilhouetteTable=function(e){var t="<table class='table'><thead><tr>"
...
for(var a=0,i=t.length;a<i;a++)for(var r=0,b=t[a].length;r<b;r++)if(null!==t[a][r]){for(var v="<input type='text' style='width: 290px;' value='",g=NNTFunctions.util.convertRange(t[a][r].point,[-2,2],[this.neuralNTree.neuralNTree.min,this.neuralNTree.neuralNTree.max]),o=0,c=g.length;o<c;o++)v+=o!==c-1?g[o].toFixed(4)+", ":g[o].toFixed(4)
