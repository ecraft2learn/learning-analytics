...
for(var s in e)e[s]=parseFloat(e[s]),s in t||isNaN(e[s])||(0==e[s]&&(e[s]=1e-6),a.push(e[s]))
...
r[n]?r[n].count++:(r[n]={},r[n].count=1,r[n].index=s)}if(Object.keys(r).length<=2)for(var l in r)this.similar[r[l].index]=!0
...
d=getInputVector(d)
for(var m=0,p=d.length;m<p;m++)j+="<td>"+d[m]+"</td>"
...
var y=O+j}b+="<br>"+JSON.stringify(d),v=v.split("\n")[0]+"\nn = "+g,a+=.3,t.nodes.update({id:u,size:a,info:y,featureCount:g,label:v})}for(var N=[],c=0;c<l.length;c++)for(var m=0;m<l.length;m++)if(c!==m&&l[c]&&l[m]&&l[c].length>1&&l[m].length>1){var T=t.silhouette(l[c],l[m])
N.push({silhouette:T,cluster:c,"compared cluster":m})}N.sort(function(e,t){return e.silhouette<t.silhouette?-1:e.silhouette>t.silhouette?1:0}),t.createSilhouetteTable(N)}else alert("No data set specified")
...
s.push({input:getInputVectorTextAllAttributes(i[n]),status:c})}for(var u=t.anomaly.outlier.probs,d=0,n=0;n<u.length;n++)d+=u[n]
u.length>0&&(d/=u.length),isNaN(d)||$("#anomaly-threshold").val(d),r="<table id='a-table' class='table'><thead><tr>"
for(var h=getKeysAllAttributes(i[0]),n=0;n<h.length;n++)r+="<th>"+h[n]+"</th>"
r+="</tr></thead><tbody>"
...
r+="</tr>"}r+="</tbody></table>",$("#anomaly-table").html(r),$("#a-table").tablesorter(),hideLoader()}),hideLoader(),analysisObjects.miners.anomaly){t.anomaly=analysisObjects.miners.anomaly,console.log(analysisObjects.miners.anomaly)
