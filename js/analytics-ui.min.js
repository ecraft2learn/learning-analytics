...
break}}function selectAnomaly(e){for(var t=e.id.split("-")[1],a=e.value,s=analysisObjects.miners.curDataSet||analysisObjects.miners.groupDataSet||analysisObjects.miners.reducedTimeDataSet||analysisObjects.miners.dataSet,r=0;r<s.length;r++)if(s[r].hiddenId==t){s[r].anomaly=a
...
c.setHours(h),c.setMinutes(m),c=a(c),r&&i&&c>=r&&c<=i?n.push(u):r&&!i&&c>=r?n.push(u):!r&&i&&c<=i&&n.push(u)}return n}return null}function pointChange(e){var t=e.id,a=$("#"+t).val(),s=t.split("_")[2],r=t.split("_")[1],i=parseFloat(a)
...
try{e=new Anomaly}catch(e){}for(var a=analysisObjects.miners.curDataSet||analysisObjects.miners.groupDataSet||analysisObjects.miners.reducedTimeDataSet||analysisObjects.miners.dataSet,s=[],r=0;r<a.length;r++){s.push({users:a[r].users,sessionId:window.sessionStorage.getItem("sessionId"),status:1===parseInt(t.decisionTree.predict(a[r]))?1:0===parseInt(t.decisionTree.predict(a[r]))?0:-1})
...
$.ajax({type:"POST",url:"https://cs.uef.fi/~tapanit/ecraft2learn/api/pilot_2/put_classification_vectors_pilot_2.php",data:n,success:function(e){console.log(e)},error:function(e){}})}if(e)for(var r=0;r<a.length;r++){var l=parseInt(e.anomaly.predict(a[r]))
l=1===l?-1:1
for(var o=0;o<s.length;o++)if(s[o].users===a[r].users){var i=1===l?0:1
$.ajax({type:"POST",url:"https://cs.uef.fi/~tapanit/ecraft2learn/api/pilot_2/put_anomaly_vectors_pilot_2.php",data:"users="+a[r].users+"&sessionId="+window.sessionStorage.getItem("sessionId")+"&anomalies="+i,success:function(e){},error:function(e){}}),s[o].status+=l}}s=function(e,t){for(var a={},s=0,r=e.length;s<r;s++)a[e[s][t]]||(a[e[s][t]]=e[s])
var i=[]
for(var n in a)i.push(a[n])
return i}(s,"users")
for(var u=-1/0,r=0;r<s.length;r++)s[r].status>u&&(u=s[r].status)
0==u&&(u=-1)
for(var r=0;r<s.length;r++){var c=s[r],n="users="+c.users+"&sessionId="+c.sessionId+"&status="+Math.floor(c.status/u*100)
...
for(var d=null,p=null,s=0,m=t.length;s<m;s++)for(var i=0,o=t[s].length;i<o;i++)if(null!==t[s][i]&&t[s][i]!==this.neuralNTree.neuralNTree.rootNode&&t[s][i]!==this.neuralNTree.neuralNTree.rootNode.left&&t[s][i]!==this.neuralNTree.neuralNTree.rootNode.right){var h=t[s][i].point
...
t[s][i].mergeIndex&&(this.mergeIndices[t[s][i].mergeIndex]=!0),this.nodes.add({id:t[s][i].index,label:t[s][i].index,level:s,info:y,points:O,size:25,shape:"square",featureCount:0,color:D,nodeHeader:j,cid:t[s][i].mergeIndex,currentPoints:O}),this.lastNodes.push({id:t[s][i].index,label:t[s][i].index,level:s,info:y,points:O,size:25,shape:"square",featureCount:0,color:D,nodeHeader:j,cid:t[s][i].mergeIndex,currentPoints:O}),null!==t[s][i].parent&&this.edges.add({from:t[s][i].index,to:t[s][i].parent.index})}var w=document.getElementById("neural-n-tree-visual"),I={nodes:this.nodes,edges:this.edges},x={edges:{smooth:{type:"cubicBezier",forceDirection:"vertical",roundness:.4}},layout:{hierarchical:{direction:"UD"}},physics:!1}
...
for(var a=analysisObjects.miners.curDataSet||analysisObjects.miners.groupDataSet||analysisObjects.miners.reducedTimeDataSet||analysisObjects.miners.dataSet,s=[],r="",i=0;i<a.length;i++){var n={}
for(var l in a[i])"users"!==l&&(l in analysisObjects.miners.ignoredAttributes||"anomaly"===l)||(n[l]=a[i][l])
s.push(n)}for(var o=[],i=0;i<s.length;i++){var u=-1==t.anomaly.anomaly.predict(s[i])
o.push({input:getInputVectorTextAllAttributes(a[i]),status:u})}r="<table id='a-table' class='table'><thead><tr>"
for(var c=getKeysAllAttributes(a[0]),i=0;i<c.length;i++)r+="<th>"+c[i]+"</th>"
r+="</tr></thead><tbody>"
for(var i=0;i<o.length;i++){r+=o[i].status?"<tr class='danger'>":"<tr class='success'>"
for(var d=0;d<o[i].input.length;d++)1e-6==o[i].input[d]&&(o[i].input[d]=0),r+="<td>"+o[i].input[d]+"</td>"
...
O.style.width="900px",O.style.height="900px",new vis.Network(O,f,y),$("#apriori-support-div").html(p),$("#freq-table").tablesorter(),$("#asso-table").tablesorter()}
