...
var p=parseInt(d[1]),m=h?parseInt(d[0])+12:parseInt(d[0])
c.setHours(m),c.setMinutes(p),c=a(c),r&&i&&c>=r&&c<=i?n.push(u):r&&!i&&c>=r?n.push(u):!r&&i&&c<=i&&n.push(u)}return n}return null}function pointChange(e){var t=e.id,a=$("#"+t).val(),s=t.split("_")[2],r=t.split("_")[1],i=parseFloat(a)
...
try{e=new Anomaly}catch(e){}for(var a=analysisObjects.miners.dataSet,s=[],r=0;r<a.length;r++)s.push({users:a[r].users,sessionId:window.sessionStorage.getItem("sessionId"),status:1===parseInt(t.decisionTree.predict(a[r]))?1:-1})
if(e){for(var r=0;r<a.length;r++){for(var i=getInputVector(a[r]),n=0;n<i.length;n++)i[n]=parseFloat(i[n])
for(var n=i.length-1;n>=0;n--)n in e.similar&&i.splice(n,1)
i=$V(i)}for(var l=e.outlier.probs,o=0,r=0;r<l.length;r++)o+=l[r]
l.length>0&&(o/=l.length)
for(var r=0;r<a.length;r++){for(var i=getInputVector(a[r]),n=0;n<i.length;n++)i[n]=parseFloat(i[n])
for(var n=i.length-1;n>=0;n--)n in e.similar&&i.splice(n,1)
i=$V(i)
var u=parseInt(e.outlier.checkIfOutlier(i,o))
u=1===u?-1:1
for(var n=0;n<s.length;n++)s[n].users===a[r].users&&(s[n].status+=u)}}s=function(e,t){for(var a={},s=0,r=e.length;s<r;s++)a[e[s][t]]||(a[e[s][t]]=e[s])
var i=[]
for(var n in a)i.push(a[n])
return i}(s,"users")
for(var c=-1/0,r=0;r<s.length;r++)s[r].status>c&&(c=s[r].status)
0==c&&(c=-1)
for(var r=0;r<s.length;r++){var d=s[r],h="users="+d.users+"&sessionId="+d.sessionId+"&status="+Math.floor(d.status/c*100)
$.ajax({method:"POST",url:"http://cs.uef.fi/~tapanit/ecraft2learn/api/pilot_2/put_student_models_pilot_2.php",data:h,success:function(){},error:function(e){console.log(e)}})}t.reset(),e&&e.reset()}var analysisObjects=analysisObjects||{}
...
for(var h=u[c],p="",m=0,g=u[c].length;m<g;m++)p+=m<g-1?u[c][m].toFixed(4)+", ":u[c][m].toFixed(4)
...
for(var f="<table class='table' style='width: 100%;'><tr>",O=getHeaders(u[c]),m=0,g=v.length;m<g;m++)f+="<th>"+O[m]+"</th>"
f+="<th>Notes</th>",f+="</tr><tr>",h=getInputVector(h)
for(var m=0;m<h.length;m++)f+="<td>"+h[m]+"</td>"
...
h=getInputVector(h)
for(var m=0,g=h.length;m<g;m++)N+="<td>"+h[m]+"</td>"
...
var f=j+N}v+="<br>"+JSON.stringify(h),y=y.split("\n")[0]+"\nn = "+b,a+=.3,t.nodes.update({id:d,size:a,points:t.nodes.get(d).currentPoints+"<hr>"+f,info:f,featureCount:b,label:y})}for(var T=[],c=0;c<l.length;c++)for(var m=0;m<l.length;m++)if(c!==m&&l[c]&&l[m]&&l[c].length>1&&l[m].length>1){var o=t.silhouette(l[c],l[m])
T.push({silhouette:o,cluster:c,"compared cluster":m})}T.sort(function(e,t){return e.silhouette<t.silhouette?-1:e.silhouette>t.silhouette?1:0}),t.createSilhouetteTable(T)}else alert("No data set specified")
...
for(var d=null,h=null,s=0,p=t.length;s<p;s++)for(var i=0,o=t[s].length;i<o;i++)if(null!==t[s][i]&&t[s][i]!==this.neuralNTree.neuralNTree.rootNode&&t[s][i]!==this.neuralNTree.neuralNTree.rootNode.left&&t[s][i]!==this.neuralNTree.neuralNTree.rootNode.right){var m=t[s][i].point
...
null===d&&(d=g),null===h&&(h=g),d<g&&(d=g),h>g&&(h=g)}this.levelCount=t.length
for(var v=getHeaders(a[0]),s=0,n=t.length;s<n;s++)for(var i=0,b=t[s].length;i<b;i++)if(null!==t[s][i]){for(var y="<input type='text' style='width: 290px;' value='",f=NNTFunctions.util.convertRange(t[s][i].point,[-2,2],[this.neuralNTree.neuralNTree.min,this.neuralNTree.neuralNTree.max]),O="<div class='nnt-point-container'>",u=0,c=f.length;u<c;u++)y+=u!==c-1?f[u].toFixed(4)+", ":f[u].toFixed(4),O+="<div class='input-group'><span class='input-group-addon'>"+v[u]+"</span><input type='text' id='node-index_"+u+"_"+t[s][i].index+"' class='node-index' onchange='pointChange(this)'></div>"
O+="</div>",y+="' placeholder='point' id='node-index_"+t[s][i].index+"' class='node-index' onchange='pointChange(this)'>"
var g=NNTFunctions.util.similarity(f,l)
g=parseInt(NNTFunctions.util.convertRange([g],[d,h],[0,255]))
var j=t[s][i].isTerminal?"Terminal "+t[s][i].index:"Node "+t[s][i].index
g<0?g=0:g>255&&(g=255)
var N=255-g,T=g,S=255-g,D="rgba("+N+","+T+","+S+",1)"
t[s][i].mergeIndex&&(this.mergeIndices[t[s][i].mergeIndex]=!0),this.nodes.add({id:t[s][i].index,label:t[s][i].index,level:s,info:y,points:O,size:25,shape:"square",featureCount:0,color:D,nodeHeader:j,cid:t[s][i].mergeIndex,currentPoints:O}),this.lastNodes.push({id:t[s][i].index,label:t[s][i].index,level:s,info:y,points:O,size:25,shape:"square",featureCount:0,color:D,nodeHeader:j,cid:t[s][i].mergeIndex,currentPoints:O}),null!==t[s][i].parent&&this.edges.add({from:t[s][i].index,to:t[s][i].parent.index})}var I=document.getElementById("neural-n-tree-visual"),x={nodes:this.nodes,edges:this.edges},w={edges:{smooth:{type:"cubicBezier",forceDirection:"vertical",roundness:.4}},layout:{hierarchical:{direction:"UD"}},physics:!1}
this.visual=new vis.Network(I,x,w),this.visual.on("click",function(t){var a=t.nodes,s=e.nodes.get(a)
...
c.length>0&&(d/=c.length),isNaN(d)||$("#anomaly-threshold").val(d),r="<table id='a-table' class='table'><thead><tr>"
